...
#include <audiotaglib/priv/ape/ValueType.hpp>

namespace audiotaglib::priv::ape {
	class ValueProcessor {
	public:
		ValueProcessor(const std::string &name);
		virtual void process(std::istream &readStream, TagMap &map, std::uint32_t size, ValueType valueType) = 0;
	protected:
		std::string name;
	};
	...
	public:
		...
		virtual void process(std::istream &readStream, TagMap &map, std::uint32_t size, ValueType valueType) override;
	};
