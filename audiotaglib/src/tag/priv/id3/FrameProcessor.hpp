...
namespace tag::priv::id3 {
	...
	public:
		...
		virtual void process(std::istream& readStream, AudioTagMap &map, std::uint32_t size) const = 0;
	protected:
		std::string name;
	};
	...
	public:
		...
		virtual void process(std::istream& readStream, AudioTagMap &map, std::uint32_t size) const override;
	};
